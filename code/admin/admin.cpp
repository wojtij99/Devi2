...
void devi::Admin(crow::App<crow::CORSHandler> &app)
{
    CROW_ROUTE(app, "/admin/addNewCompany")
        ...
        if(!exec_NOquery(&sql, {"CREATE TABLE `system_sheets` (`ID` INT NOT NULL AUTO_INCREMENT , `name` TEXT NOT NULL , `JSON` JSON NOT NULL , PRIMARY KEY (`ID`));"}, true)) 
            return crow::response(crow::CONFLICT, "{\"response\":\"Can't init DB\"}");

        if(!exec_NOquery(&sql, {"CREATE TABLE `system_pages` (`ID` INT NOT NULL AUTO_INCREMENT , `name` TEXT NOT NULL , `JSON` JSON NOT NULL , PRIMARY KEY (`ID`));"}, true)) 
            return crow::response(crow::CONFLICT, "{\"response\":\"Can't init DB\"}");
